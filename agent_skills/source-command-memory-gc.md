...
# source-command-memory-gc

Use this skill when the user asks to run the migrated source command `memory-gc`.
...
2. **On session end** — a `Stop` hook in `.Codex/settings.json` runs `python3 -m tools.memory_gc --rotate` so long sessions that wrote a lot but never crossed the cap mid-session still get cleaned up.
